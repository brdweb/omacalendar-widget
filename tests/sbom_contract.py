...
class SourceSbomTest(unittest.TestCase):
    def setUp(self) -> None:
        ...
        self.source = self.stage / "omacalendar-widget-0.1.0-rc.2"
        self.source.mkdir()
        (self.source / "manifest.json").write_text(json.dumps({"id": "org.omacalendar.widget", "version": "0.1.0-rc.2"}))
    ...
    def finish(self) -> dict:
        return module.finalize(self.document, self.stage, "0.1.0-rc.2")
    ...
    def test_complete_identity_hashes_graph_and_scope(self) -> None:
        ...
        self.assertEqual(result["packages"][0]["versionInfo"], "0.1.0-rc.2")
